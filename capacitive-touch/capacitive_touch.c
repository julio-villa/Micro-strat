...
// static app_timer_id_t TIMER5 = APP_TIMER_DEF(timer_id);
...
static void callback_function(void* p_context){
  start_capacitive_test();
}

void app_timer_tests() {
  app_timer_create(&timer_id, APP_TIMER_MODE_REPEATED, callback_function);
  app_timer_start(timer_id, 900, NULL);
}
...
void capacitive_touch_init(void) {
  ...
  nrfx_timer_compare(&TIMER4, 1, 720, 1);
  app_timer_tests();
  start_capacitive_test();
}
